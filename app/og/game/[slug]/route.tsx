...
export async function GET(_request: Request, { params }: RouteProps) {
  ...
  const image = artwork ? canonical(artwork) : null;
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #050507 0%, #160f2e 52%, #041008 100%)', color: 'white', fontFamily: 'Arial, sans-serif' }}>
        ...
        {image ? <img src={image} alt="" style={{ position: 'absolute', right: 56, top: 86, width: 440, height: 300, objectFit: 'cover', borderRadius: 28, border: '2px solid rgba(255,255,255,.22)', boxShadow: '0 30px 90px rgba(0,0,0,.55)' }} /> : null}
