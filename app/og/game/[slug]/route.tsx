...
export async function GET(_request: Request, { params }: RouteProps) {
  ...
  const game = getIndexableRegistryGames().find((item) => item.slug === slug);
  const title = game?.title || 'GR8 GAMZ';
  const category = game?.category || 'Browser game';
  const artwork = game?.artwork?.startsWith('http') ? game.artwork : undefined;
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #050507 0%, #160f2e 52%, #041008 100%)', color: 'white', fontFamily: 'Arial, sans-serif' }}>
        {artwork ? <img src={artwork} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: .32 }} /> : null}
