...
export async function POST(request) {
  const rateLimit = consumeRateLimit(request, { scope: 'auth-login', limit: 10, windowMs: 10 * 60 * 1000 });
  ...
  const result = await loginAccount(body, requestMeta(request));
  if (!result.ok) return jsonResponse(result, { status: result.status || 401 });
