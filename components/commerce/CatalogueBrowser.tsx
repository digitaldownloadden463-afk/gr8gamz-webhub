...
export default function CatalogueBrowser({ products, title = 'Products', pageType = 'category', pageSlug = 'all-products' }: { products: readonly CommerceProduct[]; title?: string; pageType?: CommercePageType; pageSlug?: string }) {
  ...
  return (
    <section className="catalogue-browser" aria-labelledby="catalogue-title">
      ...
      <div className="catalogue-toolbar">
        ...
        <label><SlidersHorizontal aria-hidden="true" /><span>Availability</span><select value={availability} onChange={(event) => setAvailability(event.target.value as 'all' | 'in-stock')}><option value="all">All catalogue products</option><option value="in-stock">Available now</option></select></label>
      ...
      {visible.length ? <div className="store-product-grid">{visible.map((product) => {
        const href = buildAffiliateUrl(product, commercePageId(pageType, pageSlug), 'catalogue-card');
        return <article className="store-product-card" key={product.slug}>
          <Link href={`/gaming-gear/products/${product.slug}`} className="store-product-card__image" aria-label={`View ${product.name} details`}><ProductVisual product={product} /></Link>
          <div className="store-product-card__body"><span className="store-product-card__brand">{product.brand}</span><h3><Link href={`/gaming-gear/products/${product.slug}`}>{product.name}</Link></h3><p>{product.keyFeatures[0] || product.shortDescription}</p><span className="store-product-card__meta">{product.availability === 'in-stock' && product.price !== null ? `$${product.price.toFixed(2)} USD - price checked ${product.sourceCheckedAt}` : 'Check current availability'} </span><div className="store-product-card__actions"><Link href={`/gaming-gear/products/${product.slug}`} className="text-link">Product details <ArrowRight size={16} aria-hidden="true" /></Link><AffiliateLink href={href} product={product} pageType={pageType} pageSlug={pageSlug} position="catalogue-card" className="cta cta--compact">View at GadgetHyper <ExternalLink size={16} aria-hidden="true" /></AffiliateLink></div></div>
        </article>;
      })}</div> : <p className="empty-catalogue">No products match those filters.</p>}
    </section>
  );
}
